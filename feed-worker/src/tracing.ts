...
const tracer = trace.getTracer("feed-worker");
 ...
 * auto-instrumentation creates inside `fn` (the Elasticsearch/DynamoDB calls
 * in event-handler.ts), link back to the request that originally enqueued
 * the event instead of starting a disconnected trace. This is the consumer
 * half of the SQS cross-service tracing gap: AWS SDK auto-instrumentation
 * spans the sendMessage/receiveMessage calls themselves, but never links
 * across the queue on its own.
 *
